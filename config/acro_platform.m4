...
case $build_os in
  ...
  linux*)
    ...
    ;;
  apple* | darwin*)
    AM_CONDITIONAL(BUILD_APPLE, true)
    AC_DEFINE(BUILD_APPLE,1,[software build os is apple])
    ;;
  solaris*)
...
case $host_os in
  ...
  linux*)
    ...
    ;;
  apple* | darwin*)
    AM_CONDITIONAL(HOST_APPLE, true)
    AM_CONDITIONAL(TARGET_APPLE, true)
    AC_DEFINE(HOST_APPLE,1,[software host will be apple])
    AC_DEFINE(TARGET_APPLE,1,[software target will be apple])
    ;;
  solaris*)
