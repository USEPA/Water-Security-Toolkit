...
#define ALLOC_BLOCK_SIZE   128000       /*(62*1024)*/
/*
...
/*
**  ioroot - Pointer to the current pool.
*/

static alloc_root_t *ioroot;
...
alloc_handle_t * AllocInit()
{
    ...
    ioroot = (alloc_root_t *) malloc(sizeof(alloc_root_t));
    if (ioroot == NULL) return(NULL);
    if ( (ioroot->first = AllocHdr()) == NULL) return(NULL);
    ioroot->current = ioroot->first;
    newpool = (alloc_handle_t *) ioroot;
    return(newpool);
}
...
{
    alloc_hdr_t  *hdr = ioroot->current;
    char         *ptr;
    ...
    */
    ...
    if (hdr->free >= hdr->end)
    {
        ...
        ptr = ioroot->current->free;
        ioroot->current->free += size;
    }
...
{
    alloc_handle_t *old = (alloc_handle_t *) ioroot;
    ioroot = (alloc_root_t *) newpool;
    return(old);
}
...
{
    ioroot->current = ioroot->first;
    ioroot->current->free = ioroot->current->block;
}
