...
*/
{
   ...
   if ((hyd_err_code=readhyd(hydtime)) != 1) return(hyd_err_code);
   if ((hyd_err_code=readhydstep(hydstep))!=1) return(hyd_err_code);
   Htime = *hydtime;
...
*/
{
   ...
   for (i=1; i<=Ntanks; i++)
   {
      ...
      /* Skip node if no WQ source */
      source = Node[i+Njuncs].S;
	  if (source == NULL || source->Type != TANK_DIRECT_MASS) continue;
      if (source->C0 == 0.0) continue;
	  ...
	  switch(Tank[i].MixModel){
		case MIX2: tankdirect2(i,massadded); break;
		case FIFO: tankdirect3(i,massadded); break;
		case LIFO: tankdirect4(i,massadded); break;
		default: tankdirect1(i,massadded); break;
	  }
   }
}

/** Complete mixing **/
void tankdirect1(int i, double massadded)
{
	if(Tank[i].V==0) {
		Tank[i].V=1;
	}
	Tank[i].M+=massadded;
	Tank[i].C=Tank[i].M/Tank[i].V;
}

/** 2 Compartment */
void tankdirect2(int i, double massadded)
{
	int k=i+Nlinks;
	Pseg s = FirstSeg[k];
	if(s->v!=0) {
		s->c = ((s->c*s->v)+massadded)/s->v;
	} else {
		// no volume in "ambient" compartment
		s=LastSeg[k];
		if(s->v!=0) {
			s->c = ((s->c*s->v)+massadded)/s->v;
		} else {
			// apparently no volume in the tank at all.
			// what to do?
			s->v=1;  // set volume to 1 cf??
			s->c=massadded;
		}
	}
	Tank[i].M+=massadded;
...
/** FIFO **/
void tankdirect3(int i, double massadded)
{
	Pseg s;
	int k=i+Nlinks;
	Tank[i].M+=massadded;
	/* add new mass to FirstSeg - the newest segment */
	if(LastSeg[k] == NULL) {
		// if there is no volume in the tank, create a new segment with 1 cubic foot volume
		addseg(k,1,0);
	}
	s=LastSeg[k];
	s->c = ((s->c*s->v)+massadded)/s->v;
...
/** LIFO **/
void tankdirect4(int i, double massadded)
{
	Pseg s;
	int k=i+Nlinks;
	Tank[i].M+=massadded;
...
/** end of additions for TANK_DIRECT source methods */
...
*/
{
   ...
   if (source->Type == MASS || source->Type==TANK_DIRECT_MASS) c /= 60.0;
   else c /= Ucf[QUALITY];
