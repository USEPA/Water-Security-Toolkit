...
*/
{
   ...
   int   result = 1;
   REAL4 *x = (REAL4 *) calloc(MAX(Nnodes,Nlinks) + 1, sizeof(REAL4));
   if ( x == NULL ) return 30700;

   if (fread(&t,sizeof(INT4),1,HydFile) < 1)  result = 30701;
   *hydtime = t;

   if (fread(x+1,sizeof(REAL4),Nnodes,HydFile) < (unsigned)Nnodes) result = 30702;
   else for (i=1; i<=Nnodes; i++) D[i] = x[i];

   if (fread(x+1,sizeof(REAL4),Nnodes,HydFile) < (unsigned)Nnodes) result = 30703;
   else for (i=1; i<=Nnodes; i++) H[i] = x[i];

   if (fread(x+1,sizeof(REAL4),Nlinks,HydFile) < (unsigned)Nlinks) result = 30704;
   else for (i=1; i<=Nlinks; i++) Q[i] = x[i];

   if (fread(x+1,sizeof(REAL4),Nlinks,HydFile) < (unsigned)Nlinks) result = 30705;
   else for (i=1; i<=Nlinks; i++) S[i] = (char) x[i];

   if (fread(x+1,sizeof(REAL4),Nlinks,HydFile) < (unsigned)Nlinks) result = 30706;
...
{
   INT4  t;
   if (fread(&t,sizeof(INT4),1,HydFile) < 1)  return(30707);
   *hydstep = t;
   return(1);
}                        /* End of readhydstep */
