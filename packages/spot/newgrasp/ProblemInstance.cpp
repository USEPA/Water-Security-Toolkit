...
void ProblemInstance::reset(void) 
{
  impactCache=NULL;
...
ProblemInstance::ProblemInstance(ProblemInstance *original, int *of, int *oc) 
{
...
void ProblemInstance::printMatrix(FILE *file) 
{
  std::cout << "***** WARNING - SHOULDN'T BE CALLING THIS!!" << std::endl;
  // TBD - REDO!
  //  int c, f;
...
void ProblemInstance::fatal(const char *func, const char *msg) 
{
...
void ProblemInstance::resetDistances(void) 
{	
...
void ProblemInstance::readImpact(FILE *input, int _p, utilib::BitArray& feasible_locations, std::map<int,int>& reduced_repn_map, bool createArrival) 
{
	...
	if(myImpactRepresentation==2) {
		imp.resize(myM);
		for(int ii=0;ii<myM;ii++) imp[ii]=DBL_MAX;
			...
			if(prevScenarioIndex != scenarioIndex && prevScenarioIndex!=-1) {
				if(myImpactRepresentation==2) {
					...
					for(int ii=0;ii<myM;ii++) {
						if(imp[ii]==DBL_MAX) imp[ii] = undetected;
					}
