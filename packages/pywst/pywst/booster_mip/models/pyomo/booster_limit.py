...
def dose_bigM_rule_lower(m,n,s):
    return m.P_DOSE_THRESHOLD_g*m.z_dosed[n,s] <= m.dose_g[n,s]

def dose_bigM_rule_upper(m,n,s):
    return m.dose_g[n,s] <= \
        m.z_dosed[n,s]*(m.P_MAX_DOSE_g[n,s]*(1+m.EPS) - m.P_DOSE_THRESHOLD_g) + \
        m.P_DOSE_THRESHOLD_g
...
def define_PD_LIMIT_model(SCENARIO_SUMMARY,
    ...
    for ndx in model.dose_g:
        model.dose_g[ndx].setub(model.P_MAX_DOSE_g[ndx]*(1+model.EPS))
        if model.dose_g[ndx].ub < model.P_DOSE_THRESHOLD_g:
            model.z_dosed[ndx].fix(0)
