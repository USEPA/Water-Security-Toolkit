...
def define_base_limit_model(SCENARIO_SUMMARY,
    ...
    # [JDS] Note: This set was originally ordered, but (as of 1/15)
    # ordered sets in Pyomo are exceedingly slow.  I checked with Gabe
    # and he sees no reason that this can't be an unordered set.
...
# Changing Big-M constraints to scale PD threshold to 1

#def dose_bigM_rule_lower(m,n,s):
#
def dose_bigM_rule_lower(m,n,s):
    if m.enablePDscaling:
        return m.P_NODE_POPULATION[n]*1.0*m.z_dosed[n,s] <= m.dose_g[n,s]
    else:
...
def dose_bigM_rule_upper(m,n,s):
    if m.enablePDscaling:
        return m.dose_g[n,s] <= \
            m.z_dosed[n,s]*m.P_NODE_POPULATION[n]*((m.P_MAX_DOSE_g[n,s]/m.P_DOSE_THRESHOLD_g)*(1+m.EPS) - 1.0) + \
            m.P_NODE_POPULATION[n]*1.0
    else:
        return m.dose_g[n,s] <= \
            m.z_dosed[n,s]*m.P_NODE_POPULATION[n]*(m.P_MAX_DOSE_g[n,s]*(1+m.EPS) - m.P_DOSE_THRESHOLD_g) + \
            m.P_NODE_POPULATION[n]*m.P_DOSE_THRESHOLD_g
...
def define_PD_LIMIT_model(SCENARIO_SUMMARY,
    ...
    # PD threshold scaling flag
    model.enablePDscaling = enablePDscaling
    ...
    for n in model.S_NZP_NODES:
        for s in model.S_SCENARIOS:
            # Max dose is also scaled by the pd threshold to account for the scaling of the pd threshold
            model.dose_g[n,s].setub(model.P_NODE_POPULATION[n]*(model.P_MAX_DOSE_g[n,s]/model.P_DOSE_THRESHOLD_g)*(1+model.EPS)) 
            if model.enablePDscaling:
                if model.dose_g[n,s].ub < model.P_NODE_POPULATION[n]:
