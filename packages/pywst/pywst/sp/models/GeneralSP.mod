...
#
set goals := {"cost", "ec", "mc", "nfd", "ns", "pe", "pd", "pk", "td", "vc", "dec", "dmc", "dpe", "dpk", "dpd", "dtd", "dvc", "pd0", "pd1", "pd2", "pd3", "pd4", "pd5", "pd6", "pd7"};
#
# Goal measures
# 
...
#
param slThreshold {g in ActiveGoals};
#
...
##
subject to limitDummySelection{g in ActiveGoals, a in Events[g], L in EventSuperLocations[g,a], l in FeasibleLocations : dummyLocation in SuperLocationMembers[L] and (exists {LL in (EventSuperLocations[g,a] diff {L})} l in SuperLocationMembers[LL]) }:
        FirstSensorForEvent[g,a,L] <= 1-s[l];

##
## Mean Goal Value
##
subject to meanGoals {(g,m) in ActiveGMPair : (m == "mean") and (g != "nfd")}:
...
## Mean nfd Goal Value
##
subject to nfdMeanGoal:
	if (("nfd","mean") in ActiveGMPair)
		then GoalValue["nfd","mean"]
		else 0.0
	>=
	if (("nfd","mean") in ActiveGMPair)
		then (sum {a in Events[objectiveGoal]} sum {L in EventSuperLocations[objectiveGoal,a]} if (dummyLocation in SuperLocationMembers[L]) then FirstSensorForEvent[objectiveGoal,a,L] else 0.0)/numEvents[objectiveGoal]
		else 0.0;	
